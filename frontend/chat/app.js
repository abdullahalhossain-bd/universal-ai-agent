...
(function () {
  ...
  function escapeHtml(str) {
    // Escapes &, <, > AND quotes: the output is used both in element text
    // and inside attribute values (src="..."), where unescaped quotes would
    // let a crafted product image_url break out and inject attributes.
  ...
  function resolveMediaUrl(url) {
    if (!url) return url;
    url = String(url).trim();
    // Only absolute http(s) URLs or same-origin-relative paths; anything
    // else (javascript:, data:, etc.) is rejected — these values come from
    // merchant feeds and end up in img src attributes.
    if (/^https?:\/\//i.test(url)) return url;
    if (/^\/[\w\-.~%]+/i.test(url)) return API_BASE + url;
    return null;
  }
  ...
  function appendMessage(role, opts) {
    ...
    if (opts.imageUrl) { var resolvedImg = resolveMediaUrl(opts.imageUrl); if (resolvedImg) { var img = document.createElement("img"); img.className = "msg-image"; img.src = resolvedImg; img.alt = "Uploaded photo"; wrap.appendChild(img); } }
  ...
  function renderProducts(products, interactionId, query) {
    ...
    products.forEach(function (p) {
      ...
      var thumbHtml = "";
      var thumbUrl = p.image_url ? resolveMediaUrl(p.image_url) : null;
      if (thumbUrl) {
        // Built via DOM so the src property can never break out of an
        // attribute, regardless of what the merchant feed contains.
        var thumb = document.createElement("img"); thumb.className = "product-thumb"; thumb.src = thumbUrl; thumb.alt = ""; thumb.loading = "lazy";
        var thumbWrap = document.createElement("div"); thumbWrap.appendChild(thumb); thumbHtml = thumbWrap.innerHTML;
      } else {
        thumbHtml = '<div class="product-thumb"></div>';
      }
