...
const AuthProvider = ({ children }) => {
  ...
  const adminLogout = async () => {
    // Server-side logout revokes the session (bumps session_version) so a
    // stolen/expired token cannot outlive the admin clicking "logout".
    try { if (getAdminToken()) await adminApi.post('/v1/admin/logout') } catch { /* local logout still proceeds */ }
