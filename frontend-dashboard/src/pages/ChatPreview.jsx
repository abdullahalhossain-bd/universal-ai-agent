...
export default function ChatPreview() {
  ...
  const uploadImage = async (e) => {
    ...
    try {
      ...
      // Routed through the API client so VITE_API_BASE_URL, the unified
      // timeout/abort handling and error parsing all apply here too.
      const result = await api.postForm('/v1/images', formData, { timeoutMs: 60000 })
  ...
  const askImageQuestion = async () => {
    ...
    try {
      ...
      // Image analysis runs a vision model: allow well beyond the default
      // 20s client timeout before aborting.
      const result = await api.post(`/v1/images/${encodeURIComponent(imageId)}/analyze`, {
        question,
        conversation_id: conversationId,
      }, { timeoutMs: 90000 })
      if (result?.conversation_id) setConversationId(result.conversation_id)
      setConversation((prev) => [
        ...
        {
          role: 'assistant',
          content: result.message || 'No visual summary returned.',
          products: Array.isArray(result.products) ? result.products : [],
          sources: Array.isArray(result.sources) ? result.sources : [],
        },
      ])
