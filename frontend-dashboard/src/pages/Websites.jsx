...
export default function Websites() {
  ...
  const pollStatus = (datasourceId) => {
    ...
    // Bound the poll: if the sync worker is down the run never reaches a
    ...
    const poll = async () => {
      if (Date.now() - startedAt > POLL_MAX_MS) {
        setError('Crawl is taking longer than expected. Check the sync worker or try a re-crawl later.')
        stopPolling()
        return
      }
      try {
