...
export default function Analytics() {
  ...
  return <div>
    <PageHeader title="Conversation intelligence" description="Understand what customers ask, what the assistant handles, and where your catalog or knowledge needs improvement." action={<div className="flex items-center gap-2"><select value={days} onChange={e => setDays(Number(e.target.value))} className="rounded-lg border border-line bg-card px-3 py-2 text-sm text-text">{PERIODS.map(p => <option key={p} value={p}>{p} days</option>)}</select><Button variant="secondary" onClick={load}><RefreshCw size={14} /> Refresh</Button></div>} />
    ...
    {loading && !data ? <div className="flex justify-center py-20 text-muted"><Spinner className="h-6 w-6" /></div> : data && <>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[1.4fr_.8fr]">
        <Card>
          ...
          <div className="mt-6 flex h-44 items-end gap-1.5 overflow-hidden">{(data.daily.series || []).map(day => <div key={day.date} title={`${day.date}: ${day.questions} questions, ${day.unanswered} no-result`} className="group flex min-w-[5px] flex-1 flex-col justify-end gap-1"><div className="rounded-t bg-accent/70 transition-all group-hover:bg-accent" style={{height: `${Math.max(3, day.questions / maxDaily * 100)}%`}} />{day.unanswered > 0 && <div className="h-1 rounded-sm bg-danger/70" />}</div>)}</div>
          <div className="mt-3 flex justify-between text-[11px] text-muted"><span>{data.daily.series?.[0]?.date || ''}</span><span>{data.daily.series?.[data.daily.series.length - 1]?.date || ''}</span></div>
        </Card>
        <Card>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        ...
        <Card>
          <div className="flex items-start justify-between"><div><h2 className="font-display text-base font-semibold">Likely knowledge & catalog gaps</h2><p className="mt-1 text-xs text-muted">Prioritized from no-result events. Review before adding content.</p></div><Badge tone="warn">Needs review</Badge></div>
