...
async function request(path, { method = 'GET', body, form, auth = true, headers = {}, token = null, timeoutMs = 20000 } = {}) {
  const finalHeaders = { ...headers }
  const normalizedMethod = method.toUpperCase()
  const resolvedToken = token !== undefined ? token : (auth ? getToken() : null)
  if (body !== undefined) finalHeaders['Content-Type'] = 'application/json'
  if (auth && resolvedToken) finalHeaders['Authorization'] = `Bearer ${resolvedToken}`
  ...
  try {
    res = await fetch(`${API_BASE}${path}`, {
      ...
      // needs to generate the multipart boundary header itself.
...
export const adminApi = {
  get: (path, opts) => request(path, { ...opts, method: 'GET', token: getAdminToken() ?? null }),
  post: (path, body, opts) => request(path, { ...opts, method: 'POST', body, token: getAdminToken() ?? null }),
  patch: (path, body, opts) => request(path, { ...opts, method: 'PATCH', body, token: getAdminToken() ?? null }),
}
