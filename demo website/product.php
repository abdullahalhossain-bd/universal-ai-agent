...
$stmt22 = $conn->prepare("INSERT INTO carts (pid, uid, product, price, quantity) VALUES (?, ?, ?, ?, ?)");
$stmt22->bind_param('iisdi', $_SESSION['product_id'], $_SESSION['id'], $_SESSION['prod-title'], $_SESSION['prod-price'], $quantity);
$stmt22->execute();
$stmt22->close();
echo "";
...
$_SESSION['product_id'] = (int)$_GET['id'];
  ...
  $_GET['id'] = $_SESSION['product_id'] ?? 0;
}
$stmt11 = $conn->prepare("SELECT * FROM products WHERE product_id = ?");
$stmt11->bind_param('i', $_SESSION['product_id']);
$stmt11->execute();
...
<img id='image-pr' src="admin/upload/<?php echo htmlspecialchars($row11['product_img']) ?>"  alt="product-img">
</div>

	<div class="detail-cont-pr">
	<h5 class="title"><?php echo htmlspecialchars($row11['product_title']) ?> <p class="date"><?php echo htmlspecialchars($row11['product_date']) ?></p> </h5>
	<p class="description-pr"><?php echo htmlspecialchars($row11['product_desc']) ?> 
