...
$stmt44 = $conn->prepare("SELECT * FROM carts WHERE uid = ?");
$stmt44->bind_param('i', $_SESSION['id']);
$stmt44->execute();
...
if($result->num_rows==1){ //if any one data found go inside it
    ...
    $passwordOk = false;
    // Supports properly hashed passwords, and transparently upgrades
    // any legacy plain-text password to a hash on successful login.
...
?>
  <head>
    <meta charset="utf-8">
    <title><?php echo htmlspecialchars($_SESSION['web-name']); ?></title>
