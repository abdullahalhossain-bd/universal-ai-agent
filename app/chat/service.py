...
class ChatService:
    ...
    async def _search_products(
        ...
        # Stop words
        # ---------------------------------
        # The shared module covers the full English/Banglish/Bengali
        # question vocabulary ("do you have earbuds?", "laptop ache?");
        # the inline extras below are product-context words kept for
        # backwards compatibility with this path's behavior.
        from app.search.stopwords import STOPWORDS as _SHARED_STOPWORDS

        stop_words = _SHARED_STOPWORDS | {
            # English
            "show",
            "find",
            "product",
            ...
            # Banglish (Roman-script Bengali) — mirrors the
            # Bengali-script stop words above. Without these,
            # e.g. "laptop ase" leaves "ase" in as a literal
            # AND-ed search term and the match always fails.
            "ase",
            "ache",
            "asche",
            "asha",
    ...
    def _has_budget_headroom(self, store_id: str) -> bool:
        """
        ...
        try:
            ...
            monthly_budget = float(store.monthly_budget or 0.0)

            if monthly_budget <= 0.0:
                return False

            committed = float(
                self.usage_repo.get_monthly_committed_usage(store_id)
                or 0.0
            )

            return committed < monthly_budget
    ...
    async def _llm_correct_search_term(
        ...
        the store has no budget headroom for an LLM call, the call
        fails, or the model's reply doesn't look like a short
        product keyword — callers must treat that as "no correction
        available" and keep the original empty result set.
        """

        # Budget gate: this LLM call happens BEFORE the mixed path's
        # atomic budget reservation, so a store with an exhausted
        # (or zero) monthly budget must never spend tokens here.
    ...
    # ------------------------------------------------------------------
    # Greetings ("Hi", "Hello", "Kemon aso", ...)
    #
    # A pure greeting must get a conversational reply, not a store-data
    # dump and not an LLM call. Detection is token-based: short messages
    # whose every word is a known greeting word (with at least one strong
    # greeting word) qualify. Longer real queries like
    # "hello ami laptop khujtechi" contain non-greeting words and fall
    # through to the normal pipeline.
    # ------------------------------------------------------------------

    _GREETING_STRONG = {
        "hi", "hii", "hiii", "hiya", "hey", "heyy", "hello", "helo",
        "hola", "yo",
        "salam", "slam", "assalamu", "asalamu", "assalamualikum",
        "alaikum", "ualikum", "adab",
        "kemon", "kmon", "kmn",
        "khobor", "khabar",
    ...
    _GREETING_FILLER = {
        "good", "are", "is", "you", "u", "r", "and", "with", "doing",
        "ki", "aso", "asen", "achen", "acho", "acen", "acis", "asis",
        "tumi", "apni", "tui", "whats", "what", "up", "it", "going",
        "bro", "vai", "vaia", "dada", "boss",
    }

    _GREETING_REPLIES = [
        (
            "আসসালামু আলাইকুম! আমি এই স্টোরের AI সহকারী। "
            "আপনাকে কীভাবে সাহায্য করতে পারি? "
            "পণ্য, দাম বা স্টক নিয়ে যেকোনো প্রশ্ন করুন — "
            "যেমন: \"laptop er dam koto?\" বা \"ki ki stock ache?\"\n\n"
            "Hello! I'm this store's AI assistant. How can I help you? "
            "Ask me anything about products, prices, or stock — "
            "e.g. \"laptop er dam koto?\" or \"what's in stock?\""
        ),
        (
            "আমি ভালো আছি, ধন্যবাদ! আপনি কেমন আছেন? "
            "স্টোর সম্পর্কে কিছু জানতে চাইলে বলুন — "
            "দাম, স্টক, বা পণ্যের তথ্য যা চান।\n\n"
            "I'm doing great, thank you! How are you? "
            "If you want to know anything about the store — "
            "prices, stock, or product info — just ask."
        ),
    ]

    def _is_greeting(self, message: str) -> bool:
        tokens = [
            t for t in re.split(r"[^a-z]+", message.lower()) if t
        ]
        if not tokens or len(tokens) > 5:
            return False
        if not all(
            t in self._GREETING_STRONG or t in self._GREETING_FILLER
            for t in tokens
        ):
            return False
        return any(t in self._GREETING_STRONG for t in tokens)

    _HOW_ARE_YOU_WORDS = {
        "kemon", "kmon", "kmn", "khobor", "khabar",
        "aso", "asen", "achen", "acho", "acen", "acis", "asis",
        "how", "are", "you", "u", "r", "doing", "ki",
    }

    def _greeting_reply(self, message: str = "") -> str:
        # "Kemon aso" / "How are you" / "Ki khobor" -> self-introduction
        # reply ("I'm fine"); everything else -> generic welcome.
        tokens = [
            t for t in re.split(r"[^a-z]+", message.lower()) if t
        ]
        if len(tokens) >= 2 and any(
            t in self._HOW_ARE_YOU_WORDS for t in tokens
        ):
            how_words = {
                "kemon", "kmon", "kmn", "khobor", "khabar",
                "how", "aso", "achen", "acho",
            }
            if any(t in how_words for t in tokens):
                return self._GREETING_REPLIES[1]
        return self._GREETING_REPLIES[0]

    def _deterministic_mixed_reply(
        self,
        *,
        conversation_id: str,
        session_id: str,
        ...
        """Answer from store data alone (no LLM composition).

        Used both when no Groq key is configured and when the
        configured keys fail at request time (invalid/revoked key,
        provider outage). The customer still sees matching products
        and knowledge links instead of a 5xx error.
        """
        top_products = self._serialize_products(products[:5])
        sources = [
            {
                "type": "website",
                "title": item.title,
        ...
        if top_products:
            ...
            parts.append(
                "আমার এই পণ্যগুলো খুঁজে পেয়েছি / "
                "Here is what I found in the store:\n"
                + product_lines
            )
        else:
            parts.append(
                "এই বিষয়ে কোনো পণ্য পাইনি / "
    ...
    async def handle(
        ...
        store_terms = await get_store_vocabulary(
            db=self.db,
            store_id=store_id,
        )
        ...
        # Greetings ("Hi", "Hello", "Kemon aso")
        #
        # Pure greetings get a friendly conversational reply without
        # touching product search, knowledge search, or the LLM.
        # ---------------------------------
        ...
        if self._is_greeting(message):
            ...
            response_message = self._greeting_reply(message)
        ...
        if planned.intent.value == "mixed":
            ...
            # ---------------------------------
            # LLM availability gate (BEFORE any budget reservation)
            # ---------------------------------
            #
            # A deployment without GROQ_API_KEY_* must still answer
            # mixed questions deterministically (product list +
            # knowledge sources) instead of raising a raw 500. The
            # no-LLM fallback also avoids reserving budget for an
            # LLM call that can never happen, so nothing leaks.
            # has_groq_keys() probes the key pool WITHOUT building
            # the provider stack, keeping the deterministic-path
            # guarantee that no LLM machinery is constructed here.
            from app.llm.groq import has_groq_keys

            if not has_groq_keys():
                return self._deterministic_mixed_reply(
                    conversation_id=conversation_id,
                    session_id=session.id,
                    products=products,
                    knowledge_results=knowledge_results,
                    note=(
                        "AI-composed answers need an LLM key; "
                        "showing store data directly."
                    ),
                )
            ...
            try:
                ...
                # The hold is released and the widget degrades to the
                # deterministic store-data answer — a customer-facing
                # chat must never dead-end on a 5xx when the products
                # and knowledge pages are already in hand. Full
                ...
                return self._deterministic_mixed_reply(
                    ...
                    knowledge_results=knowledge_results,
                    note=(
                        "AI-composed answers are temporarily "
                        "unavailable; showing store data directly. "
                        "(এআই সেবা সাময়িকভাবে বন্ধ আছে, "
                        "দোকানের তথ্য দেখানো হচ্ছে।)"
                    ),
                )
