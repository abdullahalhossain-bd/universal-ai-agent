...
def _extract_max_price(text: str) -> float | None:
    ...
    patterns = [
        r"([0-9][0-9,]*)\s*টাকার\s*মধ্যে", r"([0-9][0-9,]*)\s*টাকা\s*র মধ্যে",
        r"([0-9][0-9,]*)\s*taka\s*এর মধ্যে", r"([0-9][0-9,]*)\s*(?:er|r)\s*(?:moddhe|modhye|modhe)",
        r"([0-9][0-9,]*)\s*টাকার\s*কমে?", r"([0-9][0-9,]*)\s*টাকার\s*নিচে",
        r"under\s*(?:৳\s*)?([0-9][0-9,]*)", r"below\s*(?:৳\s*)?([0-9][0-9,]*)",
        r"within\s*(?:৳\s*)?([0-9][0-9,]*)", r"budget\s*(?:is|hocche|hobe)?\s*(?:৳\s*)?([0-9][0-9,]*)",
        r"([0-9][0-9,]*)\s*(?:max|maximum)", r"(?:৳|tk|taka)\s*([0-9][0-9,]*)",
    ]
...
def _extract_min_price(text: str) -> float | None:
    ...
    patterns = [
        ...
        r"([0-9][0-9,]*)\s*টাকার\s*উপরে", r"([0-9][0-9,]*)\s*টাকার\s*বেশি",
    ]
    for pattern in patterns:
        match = re.search(pattern, text, flags=re.IGNORECASE)
        if match:
...
def _extract_in_stock(text: str) -> bool:
    ...
    availability_question_patterns = [
        r"\bki\b.*\b(?:ase|ache)\b", r"\b(?:ase|ache)\s*\??$",
        r"\b(?:আছে|রয়েছে|রয়েছে)\s*\??$", r"\bকী\b.*\b(?:আছে|রয়েছে|রয়েছে)\b",
...
def _clean_search_terms(text: str, exclude_words: set[str] | None = None, store_terms: set[str] | None = None) -> str:
    cleaned = text
    for pattern in [
        r"[0-9][0-9,]*\s*টাকার\s*মধ্যে", r"[0-9][0-9,]*\s*taka\s*এর মধ্যে",
        r"(?:under|below|within)\s*(?:৳\s*)?[0-9][0-9,]*", r"(?:৳|tk|taka)\s*[0-9][0-9,]*",
    ]:
