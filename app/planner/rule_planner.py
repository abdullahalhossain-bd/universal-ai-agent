...
_MULTIWORD_RECOMMENDATION_CUES = ("best seller", "best-seller", "সবচেয়ে ভালো", "সবচেয়ে ভাল")

def _is_recommendation_query(text: str) -> bool:
    # Token-based matching, not substring matching: "laptop" and "desktop"
    # contain "top", so a naive `cue in lowered` check turns every laptop
    # query into a recommendation query (dropping the search term entirely).
    lowered = text.casefold()
    tokens = set(re.findall(r"[\w\u0980-\u09ff]+", lowered))
    if any(token in RECOMMENDATION_CUES for token in tokens):
        return True
    return any(cue in lowered for cue in _MULTIWORD_RECOMMENDATION_CUES)
...
def _looks_like_product_search(query: str, search_terms: str, attributes: dict, max_price: float | None, min_price: float | None) -> bool:
    ...
    tokens = [token for token in all_tokens if token not in STOP_WORDS]
    # Action words are checked against the FULL token list: "দেখাও"/"চাই"
    # (show/want) are stopwords for *search-term extraction*, but they are
    # strong product-search signals and must not be stripped before this
    # check ("কালো জুতা দেখাও চাই" must stay a product search).
    if any(token in PRODUCT_ACTION_WORDS for token in all_tokens):
        return len(all_tokens) >= 2
    ...
    # Leftover meaningful tokens ("Nike shoes", "laptop") are treated as a
    # product search even without store vocabulary. This is safe because the
    # chat pipeline guards the failure mode: greetings/chit-chat are caught
    # earlier by CONVERSATIONAL_ONLY/`_is_greeting`, and a zero-hit product
    # search falls back to learned-vocabulary classification and the
    # knowledge base instead of a bare "no products found" reply.
    return bool(non_knowledge)
