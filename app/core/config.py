...
class Settings(BaseSettings):
    ...
    admin_jwt_access_token_minutes: int = 240
    ...
    def _validate_production_settings(self):
        environments = {(self.environment or "").lower().strip(), (self.app_env or "").lower().strip()}
        ...
        if redis_host and (redis_host.endswith(".localhost") or redis_host in {"localhost", "127.0.0.1", "::1"}): raise ValueError("REDIS_URL must not point to localhost in production")
        if not [part.strip() for part in (self.cors_allow_origins or "").split(",") if part.strip()]: raise ValueError("CORS_ALLOW_ORIGINS must not be empty in production")
        if self.storage_backend.lower().strip() == "s3":
            required = {"S3_BUCKET": self.s3_bucket, "S3_REGION": self.s3_region, "S3_ACCESS_KEY_ID": self.s3_access_key_id, "S3_SECRET_ACCESS_KEY": self.s3_secret_access_key}
            missing = [name for name, value in required.items() if not value]
            if missing: raise ValueError("Missing production S3 settings: " + ", ".join(missing))
        return self
