...
def get_cors_allow_origins() -> list[str]:
    ...
    the widget's cross-origin use case. In production, an empty value
    fails closed ([] — no browser origins allowed).

    A production wildcard ("*") is a deliberate, supported choice for
    this product: the embeddable chat widget runs on arbitrary
    merchant storefront domains. It is safe because the middleware is
    configured with allow_credentials=False and the API authenticates
    via headers (x-api-key / Bearer) — no cookie-based CSRF surface.
    Never enable allow_credentials while the wildcard is active.
    """
