...
async def run_worker(redis_url:str|None=None):
    queue=SyncQueue(redis_url or settings.redis_url); consumer=f"{os.uname().nodename}-{os.getpid()}"; stopping=asyncio.Event(); loop=asyncio.get_running_loop()
        ...
        while not stopping.is_set():
            ...
            for message_id,job,attempt in entries:
                ...
                if not await queue.acquire_lock(datasource_id,token):
                    # Leave pending; it will be reclaimed after the current owner finishes.
