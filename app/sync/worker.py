...
async def run_worker(redis_url: str | None = None):
    ...
    try:
        while not stopping.is_set():
            ...
            if not entries:
                entries = await queue.read(consumer, count=1, block_ms=5000)
            for message_id, job, attempt in entries:
                ...
                if not await queue.acquire_lock(datasource_id, token):
                    logger.info("sync already running datasource=%s message=%s", datasource_id, message_id)
                    continue
                heartbeat = None
                try:
                    async def renew():
                    ...
                    if result.errors:
                        ...
                        logger.warning(
                            ...
                            datasource_id, attempt + 1, outcome, result.errors,
                        )
                    else:
                        await queue.ack(message_id)
                        # The enqueue de-dup marker must not survive a successful
                        # run, otherwise a later scheduled/manual sync can be
                        # silently dropped for up to ENQUEUE_TTL_MS.
                        await queue.clear_enqueue_marker(datasource_id)
                        logger.info(
                            "Sync ok datasource=%s created=%s updated=%s",
                            datasource_id, result.created, result.updated,
                        )
