...
def _lock_renewal_interval(lock_ttl_ms: int) -> float:
    ttl_ms = max(1000, int(lock_ttl_ms))
    return max(1.0, min(30.0, ttl_ms / 5000.0))


def _describe_lock_renewal_error(exc: BaseException) -> str:
...
async def _log_queue_stats(queue: SyncQueue, consumer: str, *, timeout: float = 3.0):
    try:
        stats = await asyncio.wait_for(queue.stats(), timeout=timeout)
    except (asyncio.TimeoutError, OSError, ConnectionError, TimeoutError) as exc:
        logger.warning("Sync queue stats skipped consumer=%s reason=%s", consumer, exc)
        return
    except Exception:
        logger.exception("Sync queue stats failed consumer=%s", consumer)
        return
    logger.info("Sync queue stats consumer=%s stats=%s", consumer, stats)
...
async def run_worker(redis_url: str | None = None):
    ...
    async def handle(message_id, job, attempt):
        ...
        try:
            ...
            renew_interval = _lock_renewal_interval(queue.lock_ttl_ms)

            async def renew():
                while not stopping.is_set():
                    await asyncio.sleep(renew_interval)
                    try:
                        renewed = await queue.renew_lock(datasource_id, token)
                    except (
                        ...
                        logger.warning(
                            "sync lock renewal timeout datasource=%s message=%s reason=%s; retrying without declaring lock lost",
                            datasource_id,
                            message_id,
                            _describe_lock_renewal_error(exc),
                        )
                        continue
                    except Exception as exc:
                        logger.warning(
                            "sync lock renewal failed datasource=%s message=%s reason=%s; aborting local execution",
                            datasource_id,
                            message_id,
                            _describe_lock_renewal_error(exc),
                        )
    ...
    async def log_stats():
        while not stopping.is_set():
            try:
                ...
                await _log_queue_stats(queue, consumer)
