"""Canonical merchant product synchronization with atomic publication."""
...
class ProductSyncService:
    ...
    def sync_rows(self, store_id, raw_rows: Iterable[dict], mapping: dict, *, full_sync=False, source_datasource_id=None):
        """Normalize and publish one sync as a single DB transaction.

        Connector reads happen before each write, but Product changes are not
        committed batch-by-batch. A failure rolls back the complete product
        mutation set, preventing half-published catalogs.
        """
        ...
        try:
            ...
            if source_datasource_id:
                q = self.db.query(func.count(Product.id)).filter(Product.store_id == store_id, Product.source_datasource_id == source_datasource_id)
                db_count = int(q.scalar() or 0)
                result.set_reconciliation(source_count=len(seen), db_count=db_count, rejected=result.skipped, duplicates=len(result.duplicate_ids))
            self.db.commit()
        except Exception as exc:
            self.db.rollback()
    ...
    def sync_from_connector(self, store_id, connector, table_name, mapping, *, full_sync=True, page_size=200, sync_state=None, sync_upper_bound=None, source_datasource_id=None):
        ...
        def _iter_rows():
            if incremental:
                ...
                while True:
                    ...
                    if timestamp_col and next_at is not None:
                        local_at, local_id = next_at, str(next_id) if next_id is not None else None
            ...
            while True:
                try:
                    rows = _fetch_page(connector, table_name, columns, limit=page_size, offset=offset)
                except Exception as exc:
                    raise RuntimeError(f"connector fetch failed at offset={offset}: {exc}") from exc
                if not rows:
                    break
                yield from rows
                if len(rows) < page_size:
    ...
    def _stock_only_upsert(self, store_id, products, result):
        ...
        existing = self.db.query(Product).filter(Product.store_id == store_id, Product.id.in_(ids)).all()
...
def _fetch_page(connector, table_name, columns, *, limit, offset):
    if hasattr(connector, "fetch_product_rows"):
        return connector.fetch_product_rows(table_name, columns, limit=limit, offset=offset)
