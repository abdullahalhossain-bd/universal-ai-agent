...
class ProductSyncService:
    ...
    def sync_rows(self,store_id,raw_rows:Iterable[dict],mapping:dict,*,full_sync=False,source_datasource_id=None):
        result=SyncResult(store_id=store_id); seen=set(); batch=[]
        try:
            for raw in raw_rows:
                try: normalized=normalize_row(raw,mapping)
                except Exception as exc: result.skipped+=1; result.errors.append(f"normalize failed: {exc}"); continue
                if normalized is None: result.skipped+=1; continue
                seen.add(normalized["id"]); batch.append(normalized)
                if len(batch)>=self.batch_size:
                    upsert_products(self.db,store_id,batch,batch_size=self.batch_size,result=result,source_datasource_id=source_datasource_id); batch=[]
            if batch: upsert_products(self.db,store_id,batch,batch_size=self.batch_size,result=result,source_datasource_id=source_datasource_id)
            if full_sync: zero_missing_stock(self.db,store_id,seen,batch_size=self.batch_size,result=result,source_datasource_id=source_datasource_id)
        except Exception as exc:
            self.db.rollback(); logger.exception("product sync failed for store %s",store_id); result.errors.append(str(exc)); raise
        return result
    def sync_from_connector(self,store_id,connector,table_name,mapping,*,full_sync=True,page_size=200,sync_state=None,sync_upper_bound=None,source_datasource_id=None):
        columns=_mapping_columns(mapping)
        if not _resolve_required(mapping,"id") or not _resolve_required(mapping,"name"):
            result=SyncResult(store_id=store_id); result.errors.append("mapping must include semantic fields 'id' and 'name'"); return result
        state=sync_state or {}; updated_col=_mapping_column(mapping,"updated_at"); created_col=_mapping_column(mapping,"created_at"); id_col=_mapping_column(mapping,"id"); timestamp_col=updated_col or created_col; watermark_at=_parse_datetime(state.get("watermark_at")); watermark_id=state.get("watermark_id")
        ...
        incremental=bool(state.get("initialized")) and bool(updated_col or created_col or id_col) and can_incremental
        upper_bound=sync_upper_bound if timestamp_col and incremental else None
        def _iter_rows():
            if incremental:
                local_at=watermark_at; local_id=watermark_id
                while True:
                    ...
                    elif id_col and next_id is not None: local_id=str(next_id)
                    else: break
            ...
            while True:
                ...
                if len(rows)<page_size: break
                offset+=page_size
        return self.sync_rows(store_id,_iter_rows(),mapping,full_sync=(full_sync and not incremental),source_datasource_id=source_datasource_id)
    def refresh_stock(self,store_id,stock_rows,mapping):
        result=SyncResult(store_id=store_id); batch=[]
    ...
    def _stock_only_upsert(self,store_id,products,result):
        ids=[p["id"] for p in products]; existing=self.db.query(Product).filter(Product.store_id==store_id,Product.id.in_(ids)).all(); by_id={r.id:r for r in existing}; pending=0
        for data in products:
            row=by_id.get(data["id"])
            if row is None: result.skipped+=1; continue
            if row.stock!=data.get("stock"): row.stock=data.get("stock"); result.updated+=1; pending+=1
            else: result.unchanged+=1
        if pending:self.db.commit()
def _resolve_required(mapping,field):
    entry=mapping.get(field); return bool(entry.get("column")) if isinstance(entry,dict) else bool(entry)
...
def _parse_datetime(value:Any):
    ...
    try:
        parsed=datetime.fromisoformat(str(value).replace("Z","+00:00")); return parsed.astimezone(timezone.utc).replace(tzinfo=None) if parsed.tzinfo else parsed
