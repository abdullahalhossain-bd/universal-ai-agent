    ...
    def sync_rows(self,store_id,raw_rows:Iterable[dict],mapping:dict,*,full_sync=False,source_datasource_id=None):
        ...
        try:
            ...
            for field,column in validation["mapping"].items():
                if column and not effective_mapping.get(field):effective_mapping[field]=column
            for raw in chain((first_raw,),raw_iter):
                ...
                if pid in seen:
                    result.record_duplicate(pid);result.skipped+=1;continue
                seen.add(pid);result.record_quality(normalized);batch.append(normalized)
...
def _mapping_columns(mapping):
    ...
    for field,entry in mapping.items():
        if field in {"_sync_state","_schema_discovery","_rest_options"}:continue
