...
class SyncResult:
 ...
 errors:list[str]=field(default_factory=list);mapping_validation:dict|None=None;data_quality:dict=field(default_factory=dict);duplicate_ids:list[str]=field(default_factory=list);duplicate_skus:list[str]=field(default_factory=list);duplicate_names:list[str]=field(default_factory=list);duplicate_candidates:list[dict]=field(default_factory=list);invalid_data:dict=field(default_factory=dict);reconciliation:dict=field(default_factory=dict);stale:dict=field(default_factory=dict);schema_drift:dict=field(default_factory=dict);repair_suggestions:list[dict]=field(default_factory=list);price_changes:list[dict]=field(default_factory=list);stock_changes:list[dict]=field(default_factory=list)
 ...
 def _invalid(self,k,p):
  b=self.invalid_data.setdefault(k,[])
 ...
 def record_quality(self,p,*,source_row=True):
  ...
  self._seen_ids.add(pid) if pid else None
 ...
 def calculate_health_score(self):
  r=self.data_quality_report();f=r["fields"];base=f["name"]["coverage_pct"]*.30+f["price"]["coverage_pct"]*.25+f["image_url"]["coverage_pct"]*.20+f["product_url"]["coverage_pct"]*.25;invalid=sum(len(v) for v in self.invalid_data.values() if isinstance(v,list));penalty=min(10,invalid*.20)+min(8,len(self.duplicate_ids)*.5+len(self.duplicate_skus)*.4)+min(5,self.skipped*.25)+min(10,float(r.get("broken_media",0))*.25);rec=self.reconciliation or {};penalty+=min(10,float(rec.get("unexplained",0) or 0)*.5) if rec and not rec.get("reconciled",False) else 0;return max(0,min(100,round(base-penalty)))
