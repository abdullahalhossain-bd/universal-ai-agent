...
class SyncResult:
 ...
 invalid_data:dict=field(default_factory=dict);invalid_counts:dict=field(default_factory=dict);reconciliation:dict=field(default_factory=dict);stale:dict=field(default_factory=dict);schema_drift:dict=field(default_factory=dict);repair_suggestions:list[dict]=field(default_factory=list)
 ...
 def _issue(self,field,item):
  ...
  if len(bucket)<1000:bucket.append(item)
 def _invalid(self,k,p):
  self.invalid_counts[k]=self.invalid_counts.get(k,0)+1;b=self.invalid_data.setdefault(k,[]);item={"id":str(p.get("id")),"name":p.get("name")}
  if len(b)<100:b.append(item)
  self._issue(k,item)
 def record_quality(self,p,*,source_row=True):
  ...
  for f in QUALITY_FIELDS:
   v=p.get(f);state="present" if v is not None and v!="" else "missing";self.data_quality["fields"][f][state]+=1
   if state=="missing":self._issue(f,{"id":pid,"name":p.get("name"),"reason":"missing"})
  ...
  if sku:
   ...
   if k in self._seen_skus:
    ...
    self._issue("duplicate_sku",{"sku":k,"id":pid,"name":p.get("name")})
 ...
 def record_duplicate(self,pid):
  ...
  self._issue("duplicate_id",{"id":v,"reason":"same source ID appeared more than once"})
 def record_price_change(self,pid,name,old,new):
  if new>old:self._price_increased+=1
 ...
 def record_stock_change(self,pid,name,old,new):
  ...
  if len(self.stock_changes)<1000:self.stock_changes.append({"id":str(pid),"name":name,"old":old,"new":new,"state":state})
