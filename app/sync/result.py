...
class SyncResult:
    ...
    duplicate_ids:list[str]=field(default_factory=list); duplicate_skus:list[str]=field(default_factory=list); duplicate_names:list[str]=field(default_factory=list); duplicate_candidates:list[dict]=field(default_factory=list)
    invalid_data:dict=field(default_factory=dict); reconciliation:dict=field(default_factory=dict); stale:dict=field(default_factory=dict); schema_drift:dict=field(default_factory=dict); repair_suggestions:list[dict]=field(default_factory=list)
    price_changes:list[dict]=field(default_factory=list); stock_changes:list[dict]=field(default_factory=list)
    ...
    def record_quality(self,product,*,source_row=True):
        if source_row:self.data_quality["source_rows"]+=1
        if product is None:self.data_quality["skipped_rows"]+=1;return
        self.data_quality["products"]+=1; pid=str(product.get("id") or "").strip()
        if pid:self._seen_ids.add(pid)
        for f in QUALITY_FIELDS:
            v=product.get(f);self.data_quality["fields"][f]["present" if v is not None and v!="" else "missing"]+=1
        name=normalize_name(product.get("name"));attrs=product.get("attributes") if isinstance(product.get("attributes"),dict) else {};sku=product.get("sku") or attrs.get("sku")
        if sku:
            k=str(sku).strip().casefold()
            if k in self._seen_skus and k not in self.duplicate_skus and len(self.duplicate_skus)<1000:self.duplicate_skus.append(k)
        ...
        if name:
            ...
            if prior is not None:
                ...
                if len(self.duplicate_candidates)<1000:
                    same_price=product.get("price") is not None and prior.get("price")==product.get("price");same_cat=normalize_name(product.get("category"))==normalize_name(prior.get("category")) and bool(product.get("category"))
                    confidence=95 if same_price and same_cat else (80 if same_price or same_cat else 60)
                    self.duplicate_candidates.append({"id_a":prior.get("id"),"id_b":pid,"name":product.get("name"),"confidence_pct":confidence,"signals":["same_name"]+(["same_price"] if same_price else [])+(["same_category"] if same_cat else [])})
            self._seen_names[name]={"id":pid,"price":product.get("price"),"category":product.get("category")}
    ...
    def record_price_change(self,product_id,name,old,new):
        if len(self.price_changes)<1000:self.price_changes.append({"id":str(product_id),"name":name,"old":old,"new":new,"direction":"increased" if new>old else "decreased"})
    def record_stock_change(self,product_id,name,old,new):
        if len(self.stock_changes)<1000:
            ...
            self.stock_changes.append({"id":str(product_id),"name":name,"old":old,"new":new,"state":state})
    def data_quality_report(self):
        fields={}
        for f,c in self.data_quality["fields"].items():
            total=c["present"]+c["missing"];fields[f]={"present":c["present"],"missing":c["missing"],"coverage_pct":round(c["present"]/total*100,2) if total else 100.0}
        d={"duplicate_id_count":len(self.duplicate_ids),"duplicate_sku_count":len(self.duplicate_skus),"possible_duplicate_name_count":len(self.duplicate_names),"possible_duplicate_count":len(self.duplicate_candidates),"sample_ids":self.duplicate_ids[:20],"sample_skus":self.duplicate_skus[:20],"sample_names":self.duplicate_names[:20],"candidates":self.duplicate_candidates[:20]}
        return {"products":self.data_quality["products"],"source_rows":self.data_quality["source_rows"],"skipped_rows":self.data_quality["skipped_rows"],"fields":fields,"duplicates":d,"invalid_data":self.invalid_data,"broken_media":self.data_quality.get("broken_media",0)}
    def calculate_health_score(self):
        ...
        invalid=sum(len(v) for v in self.invalid_data.values() if isinstance(v,list));penalty=min(10,invalid*.20)+min(8,len(self.duplicate_ids)*.5+len(self.duplicate_skus)*.4)+min(5,self.skipped*.25)+min(10,float(r.get("broken_media",0))*.25)
        rec=self.reconciliation or {};penalty+=min(10,float(rec.get("unexplained",0) or 0)*.5) if rec and not rec.get("reconciled",False) else 0
        return max(0,min(100,round(base-penalty)))
    def set_reconciliation(self,*,source_count,db_count,rejected=0,duplicates=0):
        expected=max(0,source_count-rejected-duplicates);delta=db_count-expected
        self.reconciliation={"source_count":source_count,"db_count":db_count,"expected_db_count":expected,"difference":source_count-db_count,"missing_in_db":max(0,-delta),"stale_in_db":max(0,delta),"rejected":rejected,"duplicates":duplicates,"unexplained":abs(delta),"reconciled":delta==0}
    @property
