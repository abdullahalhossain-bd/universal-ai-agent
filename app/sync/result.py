...
class SyncResult:
 ...
 def calculate_health_score(self):
  r=self.data_quality_report();f=r["fields"];base=f["name"]["coverage_pct"]*.30+f["price"]["coverage_pct"]*.25+f["image_url"]["coverage_pct"]*.20+f["product_url"]["coverage_pct"]*.25;invalid=sum(self.invalid_counts.values());dup=self._duplicate_id_count*.6+self._duplicate_sku_count*.4+max(0,self._duplicate_name_count-self._duplicate_id_count)*.05;penalty=min(12,invalid*.20)+min(10,dup)+min(5,self.skipped*.25)+min(10,float(r.get("broken_media",0))*.25);rec=self.reconciliation or {};penalty+=min(10,float(rec.get("unexplained",0) or 0)*.5) if rec and not rec.get("reconciled",False) else 0;return max(0,min(100,round(base-penalty)))
