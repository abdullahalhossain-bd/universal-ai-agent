...
def valid_source_http_url(value: object) -> bool:
    """Strict http(s) check for machine-provided product/media URLs.

    `normalize_http_url` deliberately auto-prepends "https://" for bare
    domains like "example.com" because it validates merchant WEBSITE input.
    Source rows (product_url / image_url from a merchant database, REST feed
    or crawl) are different: a bare fragment such as "bad" or "not-a-url"
    is junk data, not shorthand for a domain, and must be reported as
    malformed instead of being silently upgraded into a valid URL.
    """
    if not isinstance(value, str):
        return False
    raw = value.strip()
...
def analyze_rows(rows: list[dict]) -> dict:
    ...
    for product in rows:
        ...
        if url and not valid_source_http_url(url): add("malformed_url", product)
        image = product.get("image_url")
        if image and not valid_source_http_url(image): add("invalid_image_url", product)
