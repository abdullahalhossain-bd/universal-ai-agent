...
def zero_missing_stock(db,store_id,seen_ids,*,batch_size=200,result=None,source_datasource_id=None,commit=True):
    if result is None:result=SyncResult(store_id=store_id)
    q=db.query(Product).filter(Product.store_id==store_id);q=q.filter(Product.source_datasource_id==source_datasource_id) if source_datasource_id else q.filter(Product.source_datasource_id.is_(None))
