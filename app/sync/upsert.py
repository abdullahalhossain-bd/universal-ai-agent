...
logger = logging.getLogger("app.sync.upsert")
...
def _changed(existing, data):
    ...
    for field in _TRACKED_FIELDS:
        ...
        if field == "attributes":
            if (getattr(existing, field, {}) or {}) != (incoming or {}):
                return True
    ...
    for data in products:
        ...
        if existing is None:
            row = Product(
                ...
                source_fingerprint=incoming_fp,
            )
            db.add(row)
            db.flush()
            ...
            for field in _TRACKED_FIELDS:
                if field != "attributes" and field in data:
                    setattr(existing, field, data.get(field))
            if incoming_fp:
                existing.source_fingerprint = incoming_fp
            _persist_attributes(db, store_id, pid, data["attributes"])
            ...
            try:
                ...
                if (
                    ...
                    result.record_price_change(
                        pid, old_name, old_price, new_price
                    )
            except Exception:
                logger.warning(
                    "failed to record price change for product %s "
                    "in store %s",
                    pid,
                    store_id,
                    exc_info=True,
            ...
            try:
                ...
                if old_stock != new_stock and (
                    old_stock is not None or new_stock is not None
                ):
                    result.record_stock_change(
                        pid, old_name, old_stock, new_stock
                    )
            except Exception:
                logger.warning(
                    "failed to record stock change for product %s "
                    "in store %s",
                    pid,
                    store_id,
                    exc_info=True,
    ...
    *,
    ...
    for row in q:
        if row.stock is None or float(row.stock) != 0.0:
            ...
            result.record_stock_change(row.id, row.name, old, 0)
            pending += 1
        if commit and pending >= batch_size:
            db.commit()
            pending = 0
