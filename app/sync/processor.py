...
def _resolve_job(job, db):
    ...
    # The queue is untrusted transport: a job without a datasource id (or a
    # job whose payload disagrees with the DB row) must never let the worker
    # connect to an arbitrary connection_url or force arbitrary mapping/
    # table/full-sync values. Every legitimate enqueue path (dashboard sync
    # trigger, resync, scheduler, embeddings backfill) resolves the job
    # against the store-scoped DataSource row, which is the only authority.
    if not datasource_id:
        raise SyncJobTerminalError("sync job missing datasource_id")
    ...
    return {"store_id": store_id, "datasource_id": ds.id, "connector_type": ds.connector_type, "connection_url": decrypted, "api_base_url": ds.api_base_url, "table_name": ds.table_name, "mapping": ds.mapping or {}, "full_sync": bool(ds.full_sync), "job_type": job.get("job_type", "product_sync")}
...
async def _process_once(job):
    ...
    db = SessionLocal(); started = time.monotonic(); run = None; result = None; connector = None; execution = {"retry_group_id": job.get("_retry_group_id"), "attempt": job.get("_retry_attempt", 1)}
    try:
        ...
        if connector_type == "rest":
            ...
            # WebsiteConnector must never resolve its tenant by URL lookup:
            # bind the (already store-validated) job's store explicitly.
            if hasattr(connector, "store_id"):
                connector.store_id = store_id
        ...
        if resolved["job_type"] == "stock_refresh":
            ...
            if not result.errors and datasource_id:
                if incremental: _persist_watermark(db, datasource_id, mapping, connector, table_name, upper_bound=upper, store_id=store_id)
        ...
        if connector is not None:
            try:
                dispose = getattr(connector, "dispose", None)
                if callable(dispose): dispose()
            except Exception:
                logger.debug("connector dispose failed", exc_info=True)
        db.close()

def _persist_watermark(db, datasource_id, mapping, connector, table_name, *, upper_bound=None, store_id=None):
    ...
    # Keep the write store-scoped even though datasource ids are unique:
    # consistent with the rest of the pipeline's defense-in-depth.
    query = db.query(DataSource).filter(DataSource.id == datasource_id)
    if store_id: query = query.filter(DataSource.store_id == store_id)
    ds = query.first()
