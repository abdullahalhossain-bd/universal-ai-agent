"""Sync job processor with incremental sync, retries and persistent history."""
...
def _resolve_job(job, db):
    ...
    if ds is None:
        raise LookupError(f"datasource {datasource_id} not found for store {store_id}")
    if not ds.active:
...
def _persist_watermark(db, datasource_id, mapping, connector, table_name, *, upper_bound=None):
    ...
    if watermark.get("watermark_id") is not None:
        state["watermark_id"] = str(watermark["watermark_id"])
...
async def _auto_discover_mapping(connector, table_name, mapping):
    ...
    if tables is None and isinstance(schema, dict):
    ...
    for field, column in inferred.items():
        if not field.startswith("_") and column and not effective.get(field):
            effective[field] = column
...
def _finish_run(db, run, result: SyncResult | None, status: str, error: str | None, started: float):
    ...
    if result is not None:
        run.products_seen = int(result.data_quality_report().get("products", 0))
        run.created = result.created
        run.updated = result.updated
        run.unchanged = result.unchanged
        run.skipped = result.skipped
        run.stock_zeroed = result.stock_zeroed
        run.health_score = result.calculate_health_score()
        run.quality_report = result.data_quality_report()
        run.reconciliation = result.reconciliation or {}
    run.error = error
    db.commit()
...
async def _process_once(job):
    ...
    try:
        ...
        if not connector_type:
            raise ValueError("job missing connector_type")
        if not table_name:
            raise ValueError("job missing table_name")
        if connector_type == "rest":
            base = resolved.get("api_base_url") or resolved.get("connection_url")
            if not base:
                raise ValueError("job missing api_base_url")
            ...
            if not resolved.get("connection_url"):
                raise ValueError("job missing connection_url")
        ...
        if db.is_active and run is not None:
            db.rollback()
            try:
                _finish_run(db, run, result, "error", str(exc), started)
            except Exception:
                db.rollback()
