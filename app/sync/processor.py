...
def _start_run(db,store_id,datasource_id,sync_mode,execution=None):
 run=SyncRun(store_id=store_id,datasource_id=datasource_id,status="running",sync_mode=sync_mode,started_at=datetime.utcnow());db.add(run);db.flush();run.quality_report={"_sync_execution":execution or {}};db.commit();return run
  ...
  if resolved["job_type"]=="stock_refresh":
   ...
   if not result.errors and datasource_id:
    ...
    if (resolved["full_sync"] and not incremental) or hash_fallback:
     ...
     db_count=int(db.query(Product).filter(Product.store_id==store_id,Product.source_datasource_id==datasource_id,Product.is_active.is_(True)).count());known_stale=int(result.stale.get("stale_active",0) or 0);result.set_reconciliation(source_rows=result.data_quality.get("source_rows",0),accepted_products=result.data_quality.get("products",0),db_count=db_count,rejected=max(0,result.skipped-result._duplicate_id_count),duplicates=result._duplicate_id_count,known_stale=known_stale)
  if datasource_id:
...
async def process_sync(job):
 ...
 try:return await run_with_retry(attempt_once,attempts=4,base_delay=1.0,max_delay=15.0,logger=logger)
 except Exception as exc:
