...
async def _auto_discover_mapping(connector,table_name,mapping):
    ...
    if names:
        inferred=discover_mapping({n:None for n in names},effective)
...
async def _process_once(job):
    ...
    try:
        ...
        previous_mapping=dict(mapping);mapping=await _auto_discover_mapping(connector,table_name,mapping)
        if datasource_id and mapping!=resolved["mapping"]:
            ds=db.query(DataSource).filter(DataSource.id==datasource_id,DataSource.store_id==store_id).first()
            if ds is not None:ds.mapping=mapping;db.commit()
        state=mapping.get("_sync_state") or {};can_incremental=hasattr(connector,"fetch_product_rows_incremental") and bool(getattr(connector,"supports_incremental_sync",True));incremental=bool(state.get("initialized")) and bool(_mapping_column(mapping,"updated_at") or _mapping_column(mapping,"created_at") or _mapping_column(mapping,"id")) and can_incremental
        run=_start_run(db,store_id,datasource_id,"incremental" if incremental else "full");service=ProductSyncService(db)
        ...
        if datasource_id:
            ...
            if columns:
                analysis=schema_analysis({c:None for c in columns},previous_mapping);result.schema_drift=analysis.get("changes",[]);result.repair_suggestions=repair_suggestions({"current":analysis.get("current",{})},result.data_quality_report())
...
def _persist_watermark(db,datasource_id,mapping,connector,table_name,*,upper_bound=None):
    ...
    state["strategy"]="updated_at+id" if _mapping_column(mapping,"updated_at") else ("created_at+id" if _mapping_column(mapping,"created_at") else "id");new_mapping=dict(ds.mapping or {});new_mapping["_sync_state"]=state;ds.mapping=new_mapping;db.commit()
