...
async def widget_core_bundle() -> FileResponse:
    # Versioned content (core is stable between deploys): a short shared
    # cache lifetime removes the per-pageview re-download from every
    # storefront, while staying small enough that a fix rolls out quickly.
    # The /widget.js loader itself stays no-store (it embeds routing logic).
    return FileResponse(
        _WIDGET_PATH,
        media_type="application/javascript",
