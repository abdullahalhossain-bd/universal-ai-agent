...
# The development escape hatch lives in the same place the datasource
# guard keeps it, so ONE toggle (ALLOW_LOCAL_DATASOURCE_HOSTS) governs
# both merchant-database connections and merchant-website crawling in
# local deployments. Production leaves it unset and stays protected.
...
def is_private_host(url: str) -> bool:
    ...
    Development escape hatch: when ALLOW_LOCAL_DATASOURCE_HOSTS
    is truthy (local deployments testing against a locally
    running site), local/private hosts are allowed exactly as
    they are for merchant database connections.
    """
...
async def assert_safe_url(url: str) -> None:
    ...
    # Development escape hatch: the first-pass check above already
    # approved local/private hosts via ALLOW_LOCAL_DATASOURCE_HOSTS,
    # so skip the resolved-address sweep that would otherwise veto
    # it (e.g. 127.0.0.1). Production keeps the full sweep.
    if _local_hosts_allowed():
        return
