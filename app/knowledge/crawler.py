...
def extract_content(html:str): return parse_page(html)
...
class WebsiteCrawler:
    def __init__(self,start_url:str|None=None,max_pages:int=100,max_depth:int=3):
        self.start_url=start_url; self.max_pages=max(1,int(max_pages)); self.max_depth=max(0,int(max_depth)); self._robots=None
    ...
    async def _load_robots(self,root:str):
        ...
        try:
            text,_=await self.fetch(robots_url); rp.parse(text.splitlines()); self._robots=rp
    ...
    async def crawl(self,start_url:str|None=None)->list[dict]:
        ...
        domain=urlparse(root).netloc; await self._load_robots(root)
        visited=set(); queue=deque([(root.split("#",1)[0],0)]); pages=[]
        for url in list(await self._sitemap_urls(root,domain))[:self.max_pages]: queue.append((url,1))
        while queue and len(pages)<self.max_pages:
            url,depth=queue.popleft()
            if url in visited or not self._allowed(url): continue
            visited.add(url)
            try: html,http_status=await self.fetch(url)
            except Exception: continue
            extracted=extract_content(html); pages.append({"url":url,"title":extracted.get("title"),"content":extracted.get("content", ""),"http_status":http_status,"structured_data":extracted.get("structured_data",[])})
