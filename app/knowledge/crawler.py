...
class WebsiteCrawler:
    ...
    async def crawl(self, start_url: str | None = None) -> list[dict]:
        ...
        visited: set[str] = set()
        queue: deque[tuple[str, int]] = deque([(root.split("#", 1)[0], 0)])
        sitemap_urls = await self._sitemap_urls(root, domain)
        for url in list(sitemap_urls)[: self.max_pages]:
            queue.append((url, 1))
        pages: list[dict] = []
        while queue and len(pages) < self.max_pages:
            ...
            pages.append({"url": url, "title": extracted.get("title"), "content": extracted.get("content", ""), "http_status": http_status, "structured_data": extracted.get("structured_data", [])})
