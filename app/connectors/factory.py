...
class ConnectorFactory:
    ...
    def create(*args, **kwargs) -> object:
        if len(args) == 1 and not kwargs and isinstance(args[0], ConnectorConfig):
            return ConnectorFactory._from_config(args[0])
        if len(args) == 2 and isinstance(args[0], str) and isinstance(args[1], str):
            return ConnectorFactory._from_type_and_url(args[0], args[1])
        raise TypeError(
            "ConnectorFactory.create expects either (ConnectorConfig) or "
            "(database_type: str, url: str)."
        )
    ...
    def _from_config(config: ConnectorConfig) -> object:
        ...
        if connector_type == "mysql":
            if not config.connection_url:
                raise ValueError("MySQL connector requires `connection_url`.")
            from sqlalchemy.engine import make_url
            url = make_url(config.connection_url)
            return MySQLConnector(host=url.host or "localhost", port=url.port or 3306,
                                  username=url.username or "", password=url.password or "",
                                  database=url.database or "")
        if connector_type == "rest":
            if not config.api_base_url:
                raise ValueError("REST connector requires `api_base_url`.")
            return RESTConnector(base_url=config.api_base_url, api_key=config.api_key, options=config.options)
        if connector_type == "website":
            if not config.connection_url:
                raise ValueError("Website connector requires `connection_url`.")
    ...
    @staticmethod
    def _from_type_and_url(database_type: str, url: str) -> object:
        ...
        if normalized == "mysql":
            from sqlalchemy.engine import make_url
            parsed = make_url(url)
            return MySQLConnector(host=parsed.host or "localhost", port=parsed.port or 3306,
                                  username=parsed.username or "", password=parsed.password or "",
                                  database=parsed.database or "")
        if normalized == "rest":
            return RESTConnector(base_url=url)
        if normalized == "website":
            return WebsiteConnector(url)
