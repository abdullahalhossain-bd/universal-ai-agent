...
class WebsiteConnector:
    ...
    def __init__(self, root_url: str, *, max_pages: int = 100, max_depth: int = 3, store_id: str | None = None):
        self.root_url = root_url
        self.crawler = WebsiteCrawler(max_pages=max_pages, max_depth=max_depth)
        self._rows: list[dict] | None = None
        # Explicit tenant scope. When unset, the URL-keyed fallback below
        # would resolve whatever store most recently registered this URL —
        # a cross-tenant write risk. Job processing always sets it.
    ...
    def _store_id(self, db):
        if self.store_id:
            source = db.query(DataSource.store_id).filter(
                DataSource.store_id == self.store_id,
