...
class RESTConnector(Connector):
 ...
 def _fetch(self,params):
  ...
  for attempt in range(4):
   try:
    with httpx.Client(timeout=timeout,follow_redirects=False) as client:r=client.get(url,headers=self._headers(),params=params)
    if r.status_code in transient:
     last=httpx.HTTPStatusError(f"transient REST gateway status {r.status_code}",request=r.request,response=r)
     if attempt<3:time.sleep(2*(attempt+1));continue
     raise last
    r.raise_for_status();payload=r.json();rows=payload.get("products",payload) if isinstance(payload,dict) else payload
 ...
 async def _request_json(self,method,url,**kwargs):
  ...
  async with httpx.AsyncClient(timeout=httpx.Timeout(10.0,connect=3.0),follow_redirects=False) as client:r=await client.request(method,url,headers=self._headers(),**kwargs);r.raise_for_status();return r.json()
 @staticmethod
