...
class PostgreSQLConnector(Connector):
    # Bounded merchant-DB execution: without connect/statement timeouts a
    # hung merchant database (dropped TCP, lock wait) blocks the sync worker
    # indefinitely and starves every other queued job. 60s is generous for
    # the ≤200-row pages this connector reads.
    ...
    def __init__(self, connection_url: str):
        ...
        event.listen(self.engine, "connect", self._set_statement_timeout)

    def _set_statement_timeout(self, dbapi_connection, _connection_record):
        # Neon poolers reject statement_timeout in startup options; apply it
        # after authentication so pooled and direct Postgres endpoints work.
        cursor = dbapi_connection.cursor()
        try:
            cursor.execute(f"SET statement_timeout = {self._STATEMENT_TIMEOUT_S * 1000}")
        finally:
            cursor.close()
