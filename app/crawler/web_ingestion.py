...
logger = logging.getLogger("app.crawler.web_ingestion")
...
async def ingest_website(db, store, datasource, *, max_pages: int | None = None, max_depth: int | None = None):
    ...
    try:
        ...
        # Embeddings are generated here, inside the background worker,
        # never inline in an API request. This is best-effort: pgvector
        # and sentence-transformers are optional (see
        # app.knowledge.vector_support) and keyword search stays fully
        # functional without them, so a failure here must not fail the
        # crawl itself.
        embeddings_created = 0
        if knowledge_created or knowledge_updated:
            try:
                from app.knowledge.embedding_service import get_shared_embedding_service

                embed_service = get_shared_embedding_service()
                embeddings_created = await asyncio.to_thread(
                    embed_service.generate_missing_embeddings, store.id
                )
            except RuntimeError as exc:
                logger.info("embeddings skipped store=%s reason=%s", store.id, exc)
            except Exception:
                logger.exception("embeddings backfill failed store=%s", store.id)
