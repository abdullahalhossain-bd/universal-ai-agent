...
def extract_products_from_structured_data(structured: list[dict], page_url: str) -> list[dict]:
    ...
    for item in structured:
        ...
        rows.append({
            ...
            "product_url": url,
            "attributes": {"source": "schema.org", "source_url": page_url, "offers": offers, "aggregateRating": item.get("aggregateRating"), "additionalProperty": item.get("additionalProperty")},
        })
...
async def ingest_website(db, store, datasource, *, max_pages: int | None = None, max_depth: int = 5):
    ...
    for page in pages:
        ...
        if existing and existing.content_hash == content_hash:
            existing.status = "active"; existing.crawled_at = now; continue
        if existing:
            existing.title = page.get("title"); existing.content = content; existing.content_hash = content_hash; existing.page_type = page_type; existing.http_status = page.get("http_status"); existing.status = "active"; existing.crawled_at = now; page_id = existing.id
            db.query(KnowledgeChunk).filter(KnowledgeChunk.page_id == page_id).delete(synchronize_session=False); knowledge_updated += 1
        else:
            ...
            db.add(existing); db.flush(); page_id = existing.id; knowledge_created += 1
...
async def run_website_sync(job: dict):
    ...
    try:
        ...
        ds = db.query(DataSource).filter(DataSource.id == datasource_id, DataSource.store_id == store_id, DataSource.active.is_(True)).first()
        ...
        run = SyncRun(store_id=store_id, datasource_id=datasource_id, status="running", sync_mode="website_full", started_at=started)
        ...
        ds.last_sync_at = datetime.utcnow(); ds.last_sync_status = run.status; ds.last_sync_error = "; ".join(result["errors"]) if result["errors"] else None
        db.commit()
        ...
        db.rollback()
        if run is not None:
            run.status = "error"; run.finished_at = datetime.utcnow(); run.error = str(exc); db.add(run); db.commit()
        raise
    finally:
        db.close()
