...
(function () {
  ...
  wrap.innerHTML = '<button class="bubble" aria-label="চ্যাট খুলুন" type="button"><svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.03 2 11c0 2.28 1 4.35 2.66 5.94L4 22l5.29-1.4A11.1 11.1 0 0 0 12 21c5.52 0 10-4.03 10-9S17.52 2 12 2z"/></svg></button><div class="panel"><div class="header"><div><strong>AI Shopping Assistant</strong><small>পণ্য, দাম, স্টক ও link</small></div><button class="close" type="button" aria-label="বন্ধ করুন">×</button></div><div class="messages"></div><div class="preview"><span class="preview-name"></span><button class="tool preview-remove" type="button">সরান</button></div><div class="tools"><input class="image-input" type="file" accept="image/jpeg,image/png,image/webp" hidden><button class="tool image-button" type="button">📷 ছবি পাঠান</button></div><div class="composer"><textarea rows="1" maxlength="2000" placeholder="পণ্য, দাম বা link সম্পর্কে জিজ্ঞেস করুন…"></textarea><button class="send" type="button">পাঠান</button></div><div class="footer">Powered by Universal Commerce AI</div></div>';
  ...
  function addProducts(products) {
    ...
    products.slice(0, 10).forEach(function (p) {
      ...
      if (p.stock !== undefined && p.stock !== null) {
        ...
        stock.className = "stock" + (Number.isFinite(stockNumber) && stockNumber <= 0 ? " out" : "");
        stock.textContent = Number.isFinite(stockNumber) ? (stockNumber > 0 ? "স্টকে আছে" : "স্টক শেষ") : "স্টক জানা নেই";
  ...
  function uploadImage(file) {
    ...
    fetch(API_BASE + "/v1/images", { method: "POST", headers: { "x-api-key": API_KEY }, body: form })
      ...
      .then(function (data) {
        ...
        previewName.textContent = "ছবি প্রস্তুত: " + (file.name || "image");
        addMessage("user", "📷 " + (file.name || "ছবি") + " পাঠিয়েছি।");
        return fetch(API_BASE + "/v1/images/" + encodeURIComponent(selectedImageId) + "/analyze", {
          method: "POST", headers: { "content-type": "application/json", "x-api-key": API_KEY },
          body: JSON.stringify({ conversation_id: conversationId, question: input.value.trim() || null })
        });
      })
      .then(function (r) { if (!r.ok) throw new Error(String(r.status)); return r.json(); })
      .then(function (data) {
        if (data.conversation_id) { conversationId = data.conversation_id; try { localStorage.setItem(STORAGE_KEY, conversationId); } catch (_) {} }
        addMessage("assistant", data.message || "ছবিটি দেখেছি।");
      ...
      fetch(API_BASE + "/v1/messages/customer/" + encodeURIComponent(conversationId), { headers: { "x-api-key": API_KEY } })
        ...
        .then(function (data) {
          ...
          data.messages.forEach(function (m) {
            if (m.role !== "merchant" || !m.id || m.id === lastMerchantMessageId) return;
            lastMerchantMessageId = m.id;
            addMessage("merchant", "Store team: " + (m.content || ""));
          });
        })
        .catch(function () {});
    }, 5000);
  ...
  function sendMessage() {
    ...
    fetch(API_BASE + "/v1/chat", { method: "POST", headers: { "content-type": "application/json", "x-api-key": API_KEY }, body: JSON.stringify({ message: text, conversation_id: conversationId }) })
      ...
      .then(function (data) {
        typing.remove();
        if (data.conversation_id) { conversationId = data.conversation_id; try { localStorage.setItem(STORAGE_KEY, conversationId); } catch (_) {} }
        addMessage("assistant", data.message || ""); addProducts(data.products); startPolling();
      })
  ...
  bubble.addEventListener("click", function () {
    ...
    if (panel.classList.contains("open")) { input.focus(); startPolling(); }
  });
