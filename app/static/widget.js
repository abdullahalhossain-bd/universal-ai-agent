...
(function () {
  ...
  function escAttr(v) {
    // safeUrl guarantees http(s) but NOT quote-freedom; anything placed
    // inside src="..." must have quotes escaped to prevent attribute
    // breakout (e.g. a logo value containing '" onerror="...').
  ...
  wrap.innerHTML = '<button class="bubble" type="button" aria-label="চ্যাট খুলুন" aria-expanded="false"><svg viewBox="0 0 24 24"><path d="M20 11.5a7.5 7.5 0 0 1-7.5 7.5 8.4 8.4 0 0 1-3.6-.8L4 20l1.2-4A7.3 7.3 0 0 1 5 11.5 7.5 7.5 0 0 1 12.5 4 7.5 7.5 0 0 1 20 11.5Z"/><path d="M9 12h.01M12.5 12h.01M16 12h.01"/></svg><span class="badge"></span></button>' +
    ...
    '<header class="header"><div class="brand"><div class="avatar"><div class="avatar-inner">' + (safeUrl(LOGO) ? '<img src="' + escAttr(safeUrl(LOGO)) + '" alt="">' : '<svg viewBox="0 0 24 24"><path d="M12 2v4M12 18v4M4.9 4.9l2.8 2.8M16.3 16.3l2.8 2.8M2 12h4M18 12h4M4.9 19.1l2.8-2.8M16.3 7.7l2.8-2.8"/></svg>') + '</div><span class="online"></span></div><div class="brand-copy"><div class="brand-name"><span class="store-name"></span><span class="tag"></span></div><div class="status"><span class="dot"></span><span>Active • Instant answers</span></div></div></div><button class="close" type="button" aria-label="চ্যাট বন্ধ করুন">×</button></header>' +
  ...
  function setOpen(open) {
    ...
    else if (conversationId && conversationToken) startPolling(); // re-throttled to the slow closed-panel interval
    else stopPolling();
  }
  function renderMode() {
  ...
  // Single background poller. It used to run TWO 5s intervals against the
  // same endpoint (startPolling + startModeSync), doubling load on the
  // store-wide rate bucket; and it polled at full speed even while the
  // panel was closed, forever. Now: one loop, 5s while the panel is open
  // and 30s while closed (badge updates keep working), stopped on reset.
  var POLL_INTERVAL_OPEN_MS = 5000;
  var POLL_INTERVAL_CLOSED_MS = 30000;
  ...
  function pollOnce() {
    ...
    request(messagesUrl()).then(function (data) {
      if (!data) return;
      backgroundAuthFailures = 0;
      if (data.mode === "human" && !merchantMode) { merchantMode = true; }
  ...
  function sendMessage() {
    ...
    request("/v1/chat", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ message: text, conversation_id: conversationId, visitor_id: visitorId }) })
      .then(function (data) { typing.remove(); persist(data); add("assistant", data.message || ""); addProducts(data.products, data); startPolling(); syncMode(); })
  ...
  function upload(file) {
    ...
    fetch(API_BASE + "/v1/images", { method: "POST", headers: headers(), body: form }).then(function (r) { return r.json().catch(function () { return {}; }).then(function (d) { if (!r.ok) { var e = new Error((d && d.detail) || String(r.status)); e.status = r.status; throw e; } return d; }); })
      ...
      .then(function (data) { persist(data); add("assistant", data.message || "ছবিটি দেখেছি।"); addProducts(data.products, data); clearImage(); startPolling(); syncMode(); })
  ...
  renderMode();
  if (conversationId && conversationToken) { startPolling(); }
