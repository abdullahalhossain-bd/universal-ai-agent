...
from app.api.routes.websites import router as websites_router, legacy_router as dynamic_knowledge_router
...
def _ensure_alembic_baseline():
    ...
    if missing_tables or missing_columns:
        ...
        raise RuntimeError(
            "Alembic version table is missing and the existing schema does not match the active ORM schema; refusing to stamp head: "
            + "; ".join(details)
        )
    if os.getenv("ALLOW_ALEMBIC_BASELINE_STAMP") != "1":
        raise RuntimeError(
            "alembic_version is missing. Run `alembic upgrade head` or explicitly set ALLOW_ALEMBIC_BASELINE_STAMP=1 only after an operator has verified the entire schema."
        )
...
async def lifespan(app: FastAPI):
    ...
    if settings.run_sync_inline:
        try:
            ...
            background_tasks += [
                asyncio.create_task(run_worker(), name="inline-sync-worker"),
