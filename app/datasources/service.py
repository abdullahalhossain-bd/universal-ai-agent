...
SUPPORTED_SYNC_TYPES={"postgresql","mysql","postgres","rest","website"}; AUTO_MAP_THRESHOLD=.85
class DataSourceService:
 ...
 def create(self,store_id,*,name,connector_type,connection_url=None,api_base_url=None,table_name=None,mapping=None,active=True,full_sync=True,validate_connection=True):
  ...
  if ctype not in {"postgresql","mysql","rest","website"}: raise ValueError(f"unsupported connector_type: {connector_type}")
  if ctype=="website":
   if not connection_url: raise ValueError("connection_url required for website datasource")
  ...
  ds=DataSource(id=str(uuid.uuid4()),store_id=store_id,name=name or "default",connector_type=ctype,connection_url=get_credential_store().encrypt(connection_url) if ctype in {"postgresql","mysql"} and connection_url else connection_url if ctype=="website" else None,api_base_url=api_base_url,table_name=table_name,mapping=mapping or ({"_web":{"version":1,"strategy":"dynamic_discovery"}} if ctype=="website" else None),active=active,full_sync=full_sync,created_at=datetime.utcnow(),updated_at=datetime.utcnow())
 ...
 def update(self,store_id,datasource_id,**fields:Any):
  ...
  if ds.connector_type=="website" and new_url:
   from app.sync.quality import valid_http_url
   if not valid_http_url(new_url): raise ValueError("website URL must be a valid http(s) URL")
   fields["connection_url"]=new_url
  elif ds.connector_type=="rest" and new_url: raise ValueError("connection_url is not valid for REST connectors")
  elif ds.connector_type!="rest" and ds.connector_type!="website" and "api_base_url" in fields and new_api is not None: raise ValueError("api_base_url is only valid for REST connectors")
  if new_url and ds.connector_type in {"postgresql","mysql"}:
 ...
 def test_connection(self,connector_type,connection_url=None,api_base_url=None):
  ctype=connector_type.lower().strip(); ctype="postgresql" if ctype=="postgres" else ctype; self.last_test_error=None
  if ctype=="website":
   from app.sync.quality import valid_http_url
   return bool(connection_url and valid_http_url(connection_url))
  return self._test_connection_sync(ctype,connection_url,api_base_url=api_base_url)
 ...
 @staticmethod
 def decrypt_connection_url(ds):return get_credential_store().decrypt(ds.connection_url) if ds.connector_type in {"postgresql","mysql"} else ds.connection_url
 def build_sync_job(self,ds):
  return {"store_id":ds.store_id,"datasource_id":ds.id,"job_type":"website_sync" if ds.connector_type=="website" else "product_sync","full_sync":bool(ds.full_sync)}
