...
def _customer_row(customer: Customer | None):
    if customer is None:
        return {"customer_id": None, "name": None, "email": None, "phone": None, "first_seen_at": None, "last_seen_at": None}
    return {
...
def _sync_customer_profile(db: Session, session: ChatSession, name=None, email=None, phone=None, actor_type="customer", actor_id=None):
    if not session.customer_id:
        return None
    customer = db.query(Customer).filter(Customer.id == session.customer_id, Customer.store_id == session.store_id).first()
    if customer is None:
        return None
    changed = False
    values = {"name": name, "email": str(email).strip().lower() if email is not None else None, "phone": phone.strip() if phone is not None else None}
    for field, value in values.items():
        ...
        if field in {"email", "phone"}:
            ...
            existing = db.query(CustomerIdentity).filter(
                CustomerIdentity.store_id == session.store_id,
                CustomerIdentity.identity_type == identity_type,
                CustomerIdentity.identity_value == value,
            ).first()
            if existing is not None and existing.customer_id != customer.id:
                raise HTTPException(status_code=409, detail=f"{identity_type.title()} is already linked to another customer. Use the merchant merge flow after verification.")
            if existing is None:
                identity = CustomerIdentity(store_id=session.store_id, customer_id=customer.id, identity_type=identity_type, identity_value=value)
                db.add(identity)
                db.add(CustomerIdentityHistory(
                    store_id=session.store_id, customer_id=customer.id, action="linked",
                    identity_type=identity_type, identity_value=value, to_customer_id=customer.id,
...
def _sync_customer_from_profile(db: Session, session: ChatSession):
    if not session.customer_id or session.visitor_id == "anonymous":
        return None
    customer = db.query(Customer).filter(Customer.id == session.customer_id, Customer.store_id == session.store_id).first()
    if customer is None:
        return None
    ...
    if profile is not None:
        _sync_customer_profile(db, session, profile.name, profile.email, profile.phone, actor_type="system")
    return customer
...
def _conversation_identity(db: Session, session: ChatSession):
    ...
    if customer is not None:
        return {**_customer_row(customer), "visitor_id": session.visitor_id}
...
def _conversation_row(session: ChatSession, messages: list[ChatMessage], db: Session):
    ...
    identity = _conversation_identity(db, session)
    return {
        "conversation_id": session.conversation_key, "session_id": session.id, "visitor_id": session.visitor_id,
        "customer_id": session.customer_id, "identity": identity, "mode": session.mode or "ai",
        "mode_owner": session.mode_owner or "ai", "created_at": session.created_at, "updated_at": session.updated_at,
...
def list_conversations(auth: tuple[User, Store] = Depends(get_current_inbox_user_and_store), db: Session = Depends(get_db)):
    ...
    for session in sessions:
        ...
        identity = _conversation_identity(db, session)
        result.append({"conversation_id": session.conversation_key, "session_id": session.id, "visitor_id": session.visitor_id, "customer_id": session.customer_id, "identity": identity, "mode": session.mode or "ai", "mode_owner": session.mode_owner or "ai", "created_at": session.created_at, "updated_at": session.updated_at, "last_message": _message_row(last) if last else None})
    db.commit()
    return result
...
def list_customers(search: str | None = Query(default=None, max_length=200), auth: tuple[User, Store] = Depends(get_current_inbox_user_and_store), db: Session = Depends(get_db)):
    _user, store = auth
    query = db.query(Customer).filter(Customer.store_id == store.id)
    ...
    customers = query.order_by(Customer.last_seen_at.desc()).limit(50).all()
    return [_customer_row(customer) for customer in customers]
...
def get_customer_profile(customer_id: str, auth: tuple[User, Store] = Depends(get_current_inbox_user_and_store), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "conversations": [{"conversation_id": s.conversation_key, "session_id": s.id, "updated_at": s.updated_at, "mode": s.mode or "ai"} for s in conversations],
...
def merge_customers(payload: CustomerMergeRequest, auth: tuple[User, Store] = Depends(get_current_inbox_user_and_store), _csrf: None = Depends(require_inbox_csrf), db: Session = Depends(get_db)):
    ...
    source = _get_customer(db, store.id, payload.source_customer_id)
    target = _get_customer(db, store.id, payload.target_customer_id)
    if source is None or target is None:
        raise HTTPException(status_code=404, detail="Source or target customer not found")

    # Never auto-merge on matching email/phone. A merchant must explicitly choose this operation.
    now = datetime.utcnow()
    db.add(CustomerIdentityHistory(
        store_id=store.id, customer_id=target.id, action="merge_started",
        from_customer_id=source.id, to_customer_id=target.id, actor_type="merchant", actor_id=user.id,
        metadata_json={"source_customer_id": source.id, "target_customer_id": target.id}, created_at=now,
    ))
    ...
    for identity in db.query(CustomerIdentity).filter(CustomerIdentity.store_id == store.id, CustomerIdentity.customer_id == source.id).all():
        duplicate = db.query(CustomerIdentity).filter(
            CustomerIdentity.store_id == store.id,
        ...
        if duplicate:
            db.delete(identity)
            action = "identity_deduplicated"
        else:
            identity.customer_id = target.id
            identity.updated_at = now
            action = "identity_moved"
        db.add(CustomerIdentityHistory(
            store_id=store.id, customer_id=target.id, action=action,
            identity_type=identity.identity_type, identity_value=identity.identity_value,
            from_customer_id=source.id, to_customer_id=target.id, actor_type="merchant", actor_id=user.id,
        ))

    for session in db.query(ChatSession).filter(ChatSession.store_id == store.id, ChatSession.customer_id == source.id).all():
        session.customer_id = target.id
    ...
    db.add(CustomerIdentityHistory(
        store_id=store.id, customer_id=target.id, action="merged",
        from_customer_id=source.id, to_customer_id=target.id, actor_type="merchant", actor_id=user.id,
        metadata_json={"source_customer_id": source.id, "target_customer_id": target.id}, created_at=now,
    ))
    db.delete(source)
    db.commit()
    db.refresh(target)
    return {"merged": True, "customer": _customer_row(target), "source_customer_id": payload.source_customer_id, "target_customer_id": payload.target_customer_id}
