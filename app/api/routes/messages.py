...
def _conversation_row(session: ChatSession, messages: list[ChatMessage]):
    ...
    return {"conversation_id": session.conversation_key, "session_id": session.id, "visitor_id": session.visitor_id, "mode": session.mode or "ai", "mode_owner": session.mode_owner or "ai", "created_at": session.created_at, "updated_at": session.updated_at, "last_message": _message_row(last) if last else None, "messages": [_message_row(item) for item in messages]}
...
# Merchant Inbox endpoints are intentionally cookie-session-only. Public pk_live
# credentials are never accepted by these routes.
...
def list_conversations(
    ...
    for session in sessions:
        ...
        result.append({"conversation_id": session.conversation_key, "session_id": session.id, "visitor_id": session.visitor_id, "mode": session.mode or "ai", "mode_owner": session.mode_owner or "ai", "created_at": session.created_at, "updated_at": session.updated_at, "last_message": _message_row(last) if last else None})
    return result
...
def customer_messages(
    ...
    return {"conversation_id": conversation_id, "mode": session.mode or "ai", "mode_owner": session.mode_owner or "ai", "messages": [_message_row(item) for item in messages]}
...
def customer_set_mode(
    ...
    if requested == "ai" and session.mode_owner == "merchant":
        raise HTTPException(status_code=409, detail="The merchant currently controls this conversation. Only the merchant can resume AI.")
    if requested == "human":
...
def customer_message(
    ...
    session = _find_session(db, store_id, conversation_id)
    if session is None:
        session = ChatSession(store_id=store_id, conversation_key=conversation_id, visitor_id=payload.visitor_id.strip() or "anonymous", mode="human", mode_owner="customer")
    ...
    db.add(message); session.visitor_id = payload.visitor_id.strip() or session.visitor_id or "anonymous"
