...
async def _queue_website(url:str,name:str,db:Session,store:Store):
    require_feature(store,FEATURE_DATABASE_SYNC); ds=DataSourceService(db).create(store.id,name=name,connector_type="website",connection_url=url,full_sync=True,validate_connection=False); queue=SyncQueue(settings.redis_url)
