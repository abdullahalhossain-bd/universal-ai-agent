...
logger = logging.getLogger("app.api.websites")
...
async def website_status(datasource_id: str, db: Session = Depends(get_db), store: Store = Depends(get_current_store)):
    ...
    try:
        try:
            ...
            # PostgreSQL SyncRun remains authoritative when Redis progress is
            # temporarily unavailable; status must still serve its last run.
            logger.warning(
                "Website crawl progress unavailable store=%s datasource=%s reason=%s",
                store.id,
                datasource_id,
                exc,
            )
