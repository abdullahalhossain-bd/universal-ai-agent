...
from app.sync.quality import valid_http_url, valid_source_http_url
...
def approve_schema(datasource_id:str,payload:SchemaApprovalRequest,db:Session=Depends(get_db),store:Store=Depends(get_current_store)):
 ...
 if payload.action=="approve":
  ...
  m.pop("_pending_mapping",None);m.pop("_pending_schema",None)
  # Carry the detected candidate_hash into the approval record so the
  # processor's "approved_hash == candidate_hash" gate stays valid even if
  # re-detection produces a semantically-equal-but-differently-ordered map.
  candidate_hash=(m.get("_schema_approval") or {}).get("candidate_hash");m["_schema_approval"]={"status":"approved","approved_at":__import__("datetime").datetime.utcnow().isoformat(),"candidate_hash":candidate_hash}
 else:
