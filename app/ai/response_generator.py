...
class ResponseGenerator:
    ...
    # The grounding context is built from crawled pages and product rows.
    # Without a cap, a store with large pages can push the prompt past the
    # model's context window and every request would 400 into the
    # deterministic fallback. The cap keeps the question, the most recent
    # conversation turns and the head of the context (the formatter already
    # ranks the most relevant items first).
    ...
    async def generate(self, query: str, context, agent_config=None):
        ...
        # Bounded prompt: trim each block independently. History is trimmed
        # from the head (oldest turns dropped, latest kept); context is
        # trimmed from the tail (formatter emits highest-relevance first).
        formatted_context = formatted_context[: self._MAX_CONTEXT_CHARS]
        if len(history_text) > self._MAX_HISTORY_CHARS:
            history_text = history_text[-self._MAX_HISTORY_CHARS :]
        query = str(query)[: self._MAX_QUERY_CHARS]
