...
class GroqProvider(LLMProvider):
    ...
    async def generate(
        ...
        **kwargs: Any,
        ...
        # Single-key deployments have no key to rotate to, so a transient
        # 429/5xx/timeout currently fails the whole request on the first
        # attempt. Allow ONE bounded retry on the same key for those
        # transient classes (honoring Retry-After when provided). Larger
        # pools keep the existing rotate-immediately behavior, which is
        # more effective for per-key rate limits.
        allow_same_key_retry = len(self.api_keys) <= 1
        same_key_retry_used = False

        # The extra iteration is consumed by the bounded same-key retry;
        # without it a `continue` on retry would end the loop entirely.
        total_attempts = len(self.api_keys) + (1 if allow_same_key_retry else 0)
        ...
        for _ in range(
            ...
            try:
                ...
                if status_code >= 400:
                    ...
                    if (
                        allow_same_key_retry
                        and not same_key_retry_used
                        and status_code == 429
                    ):
                        same_key_retry_used = True
                        retry_after = response.headers.get(
                            "retry-after"
                        )
                        try:
                            delay = min(
                                max(float(retry_after), 0.5), 5.0
...
def has_groq_keys() -> bool:
    """
    Cheap availability probe: True when at least one GROQ_API_KEY_*
    is configured. Does NOT build the provider singleton (no key
    pool, no HTTP client), so callers can gate on LLM availability
    before doing any work that has side effects (budget
    reservations, etc.).
    """
