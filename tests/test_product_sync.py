...
from app.db.models import DataSource, Product, Store
...
class SyncTestBase:
    ...
    def _db(self):
        ...
        self._datasource_ids: list[str] = []
        yield
        try:
            if self._datasource_ids:
                self.db.query(DataSource).filter(
                    DataSource.id.in_(self._datasource_ids)
    ...
    def make_datasource(self, store: Store, **fields) -> DataSource:
        ds = DataSource(
            id=str(uuid.uuid4()),
            store_id=store.id,
            name=fields.pop("name", "test-ds"),
            connector_type=fields.pop("connector_type", "postgresql"),
            active=True,
            full_sync=True,
            **fields,
        )
        self.db.add(ds)
        self.db.commit()
        self._datasource_ids.append(ds.id)
        return ds
...
class TestProcessSync(SyncTestBase):
    @pytest.mark.asyncio
    async def test_job_without_datasource_id_is_terminal(self):
        # Queue payloads are untrusted transport: a job without a
        # datasource_id must be rejected outright. It used to fall back to
        # payload-supplied connection_url/table_name/mapping, which let a
        # crafted Redis message point the worker at an arbitrary database.
        result = await process_sync({"store_id": "x"})
        assert result.errors
        assert any("datasource_id" in e for e in result.errors)

    @pytest.mark.asyncio
    async def test_rest_connector_failure_is_reported(self, monkeypatch):
        """
        REST datasources ARE syncable, but a broken/unreachable REST
        endpoint must surface as a graceful per-job error (never an
        unhandled crash) — the connector's fetch failure is captured
        in result.errors and the processor keeps working.
        """
        from app.connectors.rest import RESTConnector

        def _boom(self, *args, **kwargs):
            raise RuntimeError("connector fetch failed at offset=0: boom")
