...
class TestDataSourceSync(DataSourceTestBase):
    ...
    def test_build_sync_job_shape(self):
        ...
        # The queue payload carries ONLY immutable identifiers — the
        # worker re-loads table/mapping/credentials from the database
        # so no config (or secrets) ever transit Redis.
        assert job == {
            "store_id": store.id,
            "datasource_id": ds.id,
        ...
        assert "connection_url" not in job
        assert "mapping" not in job
        assert "table_name" not in job
