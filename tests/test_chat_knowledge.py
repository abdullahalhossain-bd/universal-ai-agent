...
class TestDeterministicNoLLM(IsolatedStoreTestBase):
    """
    A pure greeting must be answered by the dedicated greeting path:
    no LLM stack construction, no provider call, a bilingual Bangla+
    English welcome reply, valid response envelope.

    (Historically the unknown-intent fallback answered greetings with
    a bare "হ্যালো! কীভাবে সাহায্য করতে পারি?"; ChatService now routes
    pure greetings through _is_greeting/_greeting_reply BEFORE intent
    planning, which is why the envelope type here is "greeting".)
    ...
    async def test_greeting_answered_without_llm_stack(self, monkeypatch):
        ...
        assert "সাহায্য" in response["message"]
    ...
    async def test_greeting_persists_user_and_assistant_messages(
        ...
        assert "সাহায্য" in messages[1].content
...
class TestBudgetExhaustedMessage(IsolatedStoreTestBase):
    ...
    async def test_budget_exhausted_returns_bangla_usage_message(
        ...
        forbid_llm_stack(monkeypatch)
        # The budget-refusal path only runs when the LLM stack is
        # configured; without keys the no-LLM deterministic fallback
        # answers before a reservation is ever attempted.
        monkeypatch.setattr("app.llm.groq.has_groq_keys", lambda: True)
    ...
    async def test_budget_exhausted_leaves_no_usage_record(self, monkeypatch):
        forbid_llm_stack(monkeypatch)
        # Same as above: pin LLM availability so the budget refusal
        # (not the no-key fallback) is what's under test.
        monkeypatch.setattr("app.llm.groq.has_groq_keys", lambda: True)
...
class TestSSRFGuard:
    ...
    @pytest.fixture(autouse=True)
    def _pin_strict_ssrf_defaults(self, monkeypatch):
        # The guard's local-host override (ALLOW_LOCAL_DATASOURCE_HOSTS)
        # is enabled suite-wide by conftest so datasource tests can point
        # at the local test database. The crawler now honours the SAME
        # toggle, so these tests must pin it off to see the strict,
